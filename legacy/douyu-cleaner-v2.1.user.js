...
(function () {
    ...
    const DEFAULT_PREFS = {
        ...
        chatCollapsed: false,
        danmuVisible: true,
        danmuOpacity: 1.0,    // 0.1 - 1.0
        danmuScale: 1.0,      // 字号 0.5 - 1.5
        danmuSpeed: 1.0,      // 0.5 - 2.0 (best-effort)
        blockScroll: false,
        blockTop: false,
        blockBottom: false,
    };
    ...
    const css = `
        ...
        /* 弹幕设置：通过 CSS 变量驱动 */
        #dy-shell-video [class*="danmu-"]{
            opacity: var(--dy-danmu-opacity, 1) !important;
        }
        #dy-shell-video [class*="danmuItem-"]{
            font-size: calc(20px * var(--dy-danmu-scale, 1)) !important;
            animation-duration: calc(var(--dy-danmu-duration, 8s) / var(--dy-danmu-speed, 1)) !important;
        }
        /* 视频弹幕开关：关掉时整个 .danmu-XXX 容器隐藏（聊天面板不受影响） */
        #dy-shell.dy-danmu-off #dy-shell-video [class*="danmu-"]{
            visibility: hidden !important;
        }
        /* 类型屏蔽（best-effort，基于斗鱼常见类名前缀） */
        #dy-shell.dy-block-scroll #dy-shell-video [class*="danmuItem-"][class*="scroll-"]{ display:none !important; }
        #dy-shell.dy-block-top    #dy-shell-video [class*="danmuItem-"][class*="top-"]   { display:none !important; }
        #dy-shell.dy-block-bottom #dy-shell-video [class*="danmuItem-"][class*="bottom-"]{ display:none !important; }
        ...
        #dy-shell-video:hover #dy-shell-controlbar,
        #dy-shell-controlbar:hover,
        #dy-shell.dy-settings-open #dy-shell-controlbar{
        ...
        #dy-shell-controlbar .dy-btn:hover{ background: rgba(255,255,255,0.12); }
        ...
        #dy-shell.dy-settings-open #dy-settings-panel{ display:block; }
        #dy-settings-panel .dy-set-title{
            font-size: 12px; color:#8a93a0; margin-bottom: 10px;
            display: flex; justify-content: space-between; align-items: center;
        }
        #dy-settings-panel .dy-set-close{
            background:none; border:none; color:#6b7077; cursor:pointer; font-size:14px;
        }
        #dy-settings-panel .dy-set-close:hover{ color:#fff; }
        #dy-settings-panel .dy-set-row{
            margin: 8px 0; display: flex; align-items: center; gap: 8px;
        }
        #dy-settings-panel .dy-set-row label{
            flex: 0 0 60px; color: #b0b6bd; font-size: 12px;
        }
        #dy-settings-panel .dy-set-row input[type="range"]{
            flex: 1 1 auto;
            -webkit-appearance: none; appearance: none;
            height: 4px; background: #2c2f36; border-radius: 2px;
            outline: none; cursor: pointer;
        }
        #dy-settings-panel .dy-set-row input[type="range"]::-webkit-slider-thumb{
            -webkit-appearance: none; appearance: none;
            width: 12px; height: 12px; border-radius: 50%; background: #4a90e2;
            cursor: pointer;
        }
        #dy-settings-panel .dy-set-row .dy-set-val{
            flex: 0 0 36px; text-align: right; color: #8a93a0; font-size: 11px;
            font-family: ui-monospace, monospace;
        }
        #dy-settings-panel .dy-set-checks{
            display: flex; gap: 12px; margin-top: 4px; flex-wrap: wrap;
        }
        #dy-settings-panel .dy-set-check{
            display: inline-flex; align-items: center; gap: 4px;
            cursor: pointer; font-size: 12px; color: #b0b6bd;
            user-select: none;
        }
        #dy-settings-panel .dy-set-check input{
            margin: 0; cursor: pointer; accent-color: #4a90e2;
        }
        #dy-settings-panel hr{
            border: none; border-top: 1px solid #2c2f36; margin: 10px 0;
        }
        ...
        #dy-shell.dy-chat-collapsed #dy-chat-expand-btn{ display:block; }
            ...
            root.innerHTML = `
                <div id="dy-chat-header">
                    ...
                    <span style="display:flex;align-items:center;gap:8px;">
                        ...
                        <button id="dy-chat-collapse-btn" title="折叠评论区">»</button>
                    </span>
                </div>
    ...
    const buildShell = () => {
        ...
        root.innerHTML = `
            <div id="dy-shell-video">
                ...
                <button id="dy-chat-expand-btn" title="展开评论区">« 评论</button>
                <div id="dy-shell-controlbar">
                    <button class="dy-btn" data-act="play" title="暂停/继续">⏸</button>
                    <button class="dy-btn" data-act="refresh" title="刷新（追到实时）">⟳</button>
                    <span class="dy-vol-wrap">
                        <button class="dy-btn" data-act="mute" title="静音切换">🔊</button>
                        <input class="dy-vol-slider" type="range" min="0" max="100" value="50" />
                    </span>
                    <div class="dy-spacer"></div>
                    <button class="dy-btn" data-act="danmu-toggle" title="视频弹幕开关">💬</button>
                    <button class="dy-btn" data-act="danmu-settings" title="弹幕设置">⚙</button>
                    <button class="dy-btn" data-act="collapse" title="折叠评论区">↔</button>
                    <button class="dy-btn" data-act="fullscreen" title="全屏">⛶</button>
                </div>
                <div id="dy-settings-panel">
                    <div class="dy-set-title">
                        <span>弹幕设置</span>
                        <button class="dy-set-close" data-act="close-settings">✕</button>
                    </div>
                    <div class="dy-set-row">
                        <label>速度</label>
                        <input type="range" data-set="speed" min="50" max="200" />
                        <span class="dy-set-val" data-show="speed"></span>
                    </div>
                    <div class="dy-set-row">
                        <label>透明度</label>
                        <input type="range" data-set="opacity" min="10" max="100" />
                        <span class="dy-set-val" data-show="opacity"></span>
                    </div>
                    <div class="dy-set-row">
                        <label>字号</label>
                        <input type="range" data-set="scale" min="50" max="150" />
                        <span class="dy-set-val" data-show="scale"></span>
                    </div>
                    <hr />
                    <div class="dy-set-row" style="flex-direction:column;align-items:flex-start;gap:6px;">
                        <label style="flex:none;">屏蔽类型</label>
                        <div class="dy-set-checks">
                            <label class="dy-set-check"><input type="checkbox" data-block="scroll" />滚动</label>
                            <label class="dy-set-check"><input type="checkbox" data-block="top" />顶部</label>
                            <label class="dy-set-check"><input type="checkbox" data-block="bottom" />底部</label>
                        </div>
                    </div>
                </div>
            </div>
        ...
        shell = {
            ...
            settingsPanel: root.querySelector('#dy-settings-panel'),
    ...
    const applyPrefs = () => {
        ...
        if (muteBtn) muteBtn.textContent = (prefs.muted || prefs.volume === 0) ? '🔇' : '🔊';
        ...
        const danmuToggleBtn = shell.controlbar.querySelector('[data-act="danmu-toggle"]');
        if (danmuToggleBtn) danmuToggleBtn.classList.toggle('dy-active', prefs.danmuVisible);
        // 折叠
        shell.root.classList.toggle('dy-chat-collapsed', !!prefs.chatCollapsed);
        // 弹幕样式 CSS 变量
        const styleRoot = shell.root.style;
        styleRoot.setProperty('--dy-danmu-opacity', String(prefs.danmuOpacity));
        styleRoot.setProperty('--dy-danmu-scale', String(prefs.danmuScale));
        styleRoot.setProperty('--dy-danmu-speed', String(prefs.danmuSpeed));
        // 屏蔽类型
        shell.root.classList.toggle('dy-block-scroll', !!prefs.blockScroll);
        shell.root.classList.toggle('dy-block-top', !!prefs.blockTop);
        shell.root.classList.toggle('dy-block-bottom', !!prefs.blockBottom);
        // settings 面板里的 input 值
        const sp = shell.settingsPanel;
        const setSlider = (k, val) => { const el = sp.querySelector('[data-set="' + k + '"]'); if (el) el.value = String(val); };
        const setShow = (k, txt) => { const el = sp.querySelector('[data-show="' + k + '"]'); if (el) el.textContent = txt; };
        setSlider('speed', Math.round(prefs.danmuSpeed * 100));
        setShow('speed', Math.round(prefs.danmuSpeed * 100) + '%');
        setSlider('opacity', Math.round(prefs.danmuOpacity * 100));
        setShow('opacity', Math.round(prefs.danmuOpacity * 100) + '%');
        setSlider('scale', Math.round(prefs.danmuScale * 100));
        setShow('scale', Math.round(prefs.danmuScale * 100) + '%');
        ['scroll', 'top', 'bottom'].forEach(k => {
            const cb = sp.querySelector('[data-block="' + k + '"]');
            const prefKey = 'block' + k[0].toUpperCase() + k.slice(1);
            if (cb) cb.checked = !!prefs[prefKey];
        });
    };
    ...
    const bindControls = () => {
        ...
        cb.addEventListener('click', (e) => {
            ...
            switch (act) {
                ...
                case 'danmu-settings': {
                    shell.root.classList.toggle('dy-settings-open');
                    break;
                }
                case 'collapse': {
        ...
        const updatePlayIcon = () => {
            ...
            if (playBtn && vid) playBtn.textContent = vid.paused ? '▶' : '⏸';
        };
        const vid0 = v();
        if (vid0) {
        ...
        sp.addEventListener('click', (e) => {
            const close = e.target.closest('[data-act="close-settings"]');
            if (close) shell.root.classList.remove('dy-settings-open');
        });
        // 点击 settings 面板外面关闭
        document.addEventListener('mousedown', (e) => {
            if (!shell || !shell.root.classList.contains('dy-settings-open')) return;
            if (sp.contains(e.target)) return;
            if (e.target.closest('[data-act="danmu-settings"]')) return; // 切换按钮自己处理
            shell.root.classList.remove('dy-settings-open');
        }, true);
        ...
        sp.addEventListener('input', (e) => {
            ...
            if (!sl) return;
            const k = sl.dataset.set;
            const pct = (+sl.value) / 100;
            if (k === 'speed') setPref('danmuSpeed', pct);
            if (k === 'opacity') setPref('danmuOpacity', pct);
            if (k === 'scale') setPref('danmuScale', pct);
        });
        // settings 屏蔽 checkboxes
        sp.addEventListener('change', (e) => {
            const cb2 = e.target.closest('[data-block]');
            if (!cb2) return;
            const k = cb2.dataset.block;
            const prefKey = 'block' + k[0].toUpperCase() + k.slice(1);
            setPref(prefKey, cb2.checked);
        });
    ...
    const mount = async () => {
        ...
        pickHighestQuality();
        startLatencyChase();
        console.log('[Douyu Cleaner] v2.1 shell mounted');
    };
    const unmount = () => {
        stopLatencyChase();
    ...
    console.log('[Douyu Cleaner] v2.1.0 已加载');
})();
