...
(function () {
    ...
    const css = `
        ...
        #dy-settings-panel, #dy-settings-panel *,
        ...
        root.innerHTML = `
            <div id="dy-shell-video">
                ...
                <div id="dy-shell-controlbar">
                    ...
                    <button class="dy-btn" data-act="danmu-toggle" title="视频弹幕开关">${SVG.danmu}</button>
    ...
    const applyPrefs = () => {
        ...
        if (danmuToggleBtn) danmuToggleBtn.classList.toggle('dy-off', !prefs.danmuVisible);
        // 折叠
    ...
    const bindControls = () => {
        ...
        const scheduleHide = () => {
            ...
            hideTimer = setTimeout(() => {
                if (!shell) return;
                if (shell.root.classList.contains('dy-settings-open')) return; // 设置打开时不隐
                if (cb.matches(':hover') || sp.matches(':hover')) return;       // hover 状态不隐
        ...
        const hideUINow = () => {
            if (!shell) return;
            if (shell.root.classList.contains('dy-settings-open')) return;
            clearTimeout(hideTimer);
            shell.root.classList.remove('dy-ui-active');
        };
        ...
        // hover controlbar / settings 期间始终保持显示
        ...
        sp.addEventListener('mouseenter', () => { clearTimeout(hideTimer); shell.root.classList.add('dy-ui-active'); });
        sp.addEventListener('mouseleave', scheduleHide);
        // 初次挂载短暂显示提示一下 UI 存在
        showUI();

        // === 双击视频区切换全屏（避免点到工具条/设置/expand 按钮） ===
        shell.videoHost.addEventListener('dblclick', (e) => {
            if (e.target.closest('#dy-shell-controlbar, #dy-settings-panel, #dy-chat-expand-btn')) return;
            if (document.fullscreenElement) document.exitFullscreen().catch(() => {});
            else shell.root.requestFullscreen().catch(() => {});
        });
    ...
    const mount = async () => {
        ...
        pickHighestQuality();
        startLatencyChase();
        console.log('[Douyu Cleaner] v2.2 shell mounted');
    };
    const unmount = () => {
        stopLatencyChase();
    ...
    console.log('[Douyu Cleaner] v2.2.0 已加载');
})();
