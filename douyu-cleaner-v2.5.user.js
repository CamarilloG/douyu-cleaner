...
(function () {
    ...
    /* 用户偏好持久化（localStorage）
       注意：弹幕显示/速度/不透明度/字号/屏蔽类型 这些设置已交还给原生
       showdanmuWrap 浮层（reparent 到 controlbar），由斗鱼自己持久化，
       我们不在这里重复存储。 */
    ...
    const css = `
        ...
        /* 弹幕样式不再用 CSS 变量驱动（v2.2 的"弹幕消失"根因），完全交还原生。
           但 React 渲染时把弹幕容器 inline style 写死为原小窗尺寸（如 988×498），
           reparent 后我们的视频区是 1090×754，React 的 ResizeObserver 响应有
           几秒延迟（甚至有时漏触发）。所以保留一条最小 CSS：强制弹幕容器铺满
           父级（仅作用于容器层，:not 排除子弹幕 .danmuItem 让其内部定位继续生效）。 */
        ...
        #dy-shell.dy-native-setting-open #dy-shell-controlbar{
        ...
        /* === 原生 showdanmuWrap reparent 后的适配 ===
           我们 reparent 进 controlbar 的原生 wrap 自带 i 图标 + setting 浮层。
           只需把它在 controlbar 里占位正常、setting 浮层弹出位置合适即可。 */
        #dy-native-danmu-slot{
            display: inline-flex; align-items: center;
            min-width: 36px; min-height: 36px;
            position: relative;
        }
        #dy-native-danmu-slot [class*="showdanmuWrap"]{
        ...
        #dy-native-danmu-slot [class*="showdanmuWrap"] > i{
            width: 26px; height: 26px;
            display: inline-flex; align-items:center; justify-content:center;
            cursor: pointer;
        }
        #dy-native-danmu-slot [class*="showdanmuWrap"] > i svg{
            width: 26px; height: 26px; display: block;
        }
        #dy-native-danmu-slot [class*="showdanmuWrap"]:hover{
            background: rgba(255,255,255,0.12); border-radius: 4px;
        }
        /* 原生 setting 浮层：让它继续用原生 absolute 计算 + panelFill 桥接 hitbox。
           我们只确保：
             1) z-index 足够高，不被 controlbar 内其他元素覆盖
             2) 父级（slot / controlbar）overflow:visible，浮层不被裁掉 */
        #dy-native-danmu-slot [class*="setting-"]{
    ...
    let danmuRebindTimer = null;

    const buildShell = () => {
        if (document.getElementById('dy-shell')) return;
        const root = document.createElement('div');
        root.id = 'dy-shell';
        // 必须挂到 React root container (#root) 内：React 18 把事件代理绑在 root
        // container 上而非 document，搬进我们 shell 内的原生组件需要 shell 仍在
        // React root 子树才能收到合成事件（onClick 等）。
        // fallback 到 body 只是兜底（如斗鱼某天换了 root id）。
        root.innerHTML = `
            <div id="dy-shell-video">
                ...
                <div id="dy-shell-controlbar">
                    ...
                    <!-- 原生弹幕开关 + 完整设置浮层会被 reparent 到这里 -->
                    <span id="dy-native-danmu-slot"></span>
        ...
        shell = {
            ...
            nativeDanmuSlot: root.querySelector('#dy-native-danmu-slot'),
    ...
    // reparent 原生 [class*="showdanmuWrap-"] 到 controlbar 的 slot。
    // 它自带：点击切换弹幕开关 + hover 弹出完整设置浮层（速度/不透明度/字号/位置/屏蔽 全套）。
    // 由原生 React 维护状态和持久化，我们零干预。
    let originalDanmuWrapParent = null;
    let originalDanmuWrapNext = null;
    const reparentNativeDanmuWrap = async () => {
        if (!shell || !shell.nativeDanmuSlot) return false;
        const wrap = await waitFor('[class*="showdanmuWrap"]', 20000);
        if (!wrap) return false;
        if (wrap.parentElement === shell.nativeDanmuSlot) return true;
        originalDanmuWrapParent = wrap.parentElement;
        originalDanmuWrapNext = wrap.nextSibling;
        shell.nativeDanmuSlot.appendChild(wrap);
        return true;
    };
    ...
    const bindControls = () => {
        ...
        // === 鼠标静置自动隐藏 UI ===
        // 原生 setting 浮层 hover 期间会一直在 DOM 内，我们通过它的存在性来判断"是否在调设置"。
        const isNativeSettingOpen = () => {
            // hover showdanmuWrap 时，setting 浮层会有可见尺寸
            const setting = shell.nativeDanmuSlot && shell.nativeDanmuSlot.querySelector('[class*="setting-"]');
            if (!setting) return false;
            const r = setting.getBoundingClientRect();
        ...
        const scheduleHide = () => {
            ...
            hideTimer = setTimeout(() => {
                if (!shell) return;
                if (cb.matches(':hover')) return;        // hover controlbar 不隐
                if (isNativeSettingOpen()) return;       // 原生设置浮层弹出时不隐
    ...
    const mount = async () => {
        ...
        bindChat();
        pickHighestQuality();
        reparentNativeDanmuWrap();
        startLatencyChase();
        // 流状态变化（断流/重连）时 React 会重建 showdanmuWrap，需要重 reparent
        if (danmuRebindTimer) clearInterval(danmuRebindTimer);
        danmuRebindTimer = setInterval(() => {
            if (!shell || !shell.nativeDanmuSlot) return;
            const slotHas = shell.nativeDanmuSlot.querySelector('[class*="showdanmuWrap"]');
            if (slotHas) return;
            const stray = document.querySelector('[class*="showdanmuWrap"]');
            if (stray) shell.nativeDanmuSlot.appendChild(stray);
        }, 3000);
        console.log('[Douyu Cleaner] v2.5 shell mounted');
    };
    const unmount = () => {
        stopLatencyChase();
        if (danmuRebindTimer) { clearInterval(danmuRebindTimer); danmuRebindTimer = null; }
        // 复原原生 showdanmuWrap
        const wrap = document.querySelector('[class*="showdanmuWrap"]');
        if (wrap && originalDanmuWrapParent && originalDanmuWrapParent.isConnected) {
            try { originalDanmuWrapParent.insertBefore(wrap, originalDanmuWrapNext); } catch (e) {}
        }
        originalDanmuWrapParent = null;
        originalDanmuWrapNext = null;
        // 复原 player case
    ...
    console.log('[Douyu Cleaner] v2.5.0 已加载');
})();
