...
(function () {
    ...
    /* 用户偏好持久化（localStorage）
       v2.6 教训：reparent 原生 showdanmuWrap 失败（斗鱼用 React 15/16 + 原生 wrap 上
       带 `removed-XXX` modifier 让按钮失效）。改回自建开关 + CSS visibility 控制。
       速度/不透明度/字号 这些精细设置先不做（v2.2 自建 CSS 变量驱动曾让弹幕消失）。 */
    ...
    const loadPrefs = () => {
        try {
            ...
            // 只信任 DEFAULT_PREFS 里已知的 key，清洗掉 v2.1/v2.2 残留（如 danmuOpacity:0 之类的"脏值"会让弹幕看不见）。
            const clean = {};
    ...
    const css = `
        ...
        /* 飞行弹幕容器：强制铺满视频区（绕开 React ResizeObserver 延迟）。
           只针对容器层（:not 排除子弹幕 .danmuItem-XXX 保留它们 inline 定位、
           排除 controlbar 上的 showdanmuWrap/sendDanmu/simpleDanmu 等无关元素）。 */
        ...
        /* 弹幕开关：关掉时整个飞行弹幕层不可见（聊天面板镜像不受影响） */
        #dy-shell.dy-danmu-off #dy-shell-video [class*=" danmu-"]:not([class*="danmuItem"]):not([class*="showdanmu"]):not([class*="danmuModels"]):not([class*="danmuTips"]):not([class*="danmuReport"]):not([class*="sendDanmu"]):not([class*="simpleDanmu"]),
        #dy-shell.dy-danmu-off #dy-shell-video [class^="danmu-"]:not([class*="danmuItem"]):not([class*="showdanmu"]):not([class*="danmuModels"]):not([class*="danmuTips"]):not([class*="danmuReport"]):not([class*="sendDanmu"]):not([class*="simpleDanmu"]),
        #dy-shell.dy-danmu-off #dy-shell-video .DanmuEffectDom{
    ...
    const buildShell = () => {
        ...
        const root = document.createElement('div');
        root.id = 'dy-shell';
        // v2.6 起：dy-shell 挂回 body 直接子节点（不再依赖 React root 子树承接事件代理，
        // 因为 v2.6 不再 reparent 任何原生 React 控件，只复用 #js-player-video-case 这棵子树）。
        root.innerHTML = `
    ...
    const bindControls = () => {
        ...
        // === 双击视频区切换全屏（避开 controlbar / expand 按钮） ===
    ...
    const mount = async () => {
        ...
        pickHighestQuality();
        startLatencyChase();
        console.log('[Douyu Cleaner] v2.6 shell mounted');
    };
    const unmount = () => {
        stopLatencyChase();
        // 复原 player case
    ...
    console.log('[Douyu Cleaner] v2.6.0 已加载');
})();
